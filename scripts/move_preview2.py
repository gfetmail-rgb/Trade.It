...
# Designer-created preview controls.
d = Path("PortfolioManagementForm.Designer.cs")
s = d.read_text(encoding="utf-8")

if "private GroupBox previewGroup;" not in s:
    fields = ["        private GroupBox previewGroup;", "        private DataGridView previewGrid;"]
    fields += [f"        private DataGridViewTextBoxColumn previewColumn{i};" for i in range(1, 19)]
    s = s.replace("        private Button closeButton;\n", "        private Button closeButton;\n" + "\n".join(fields) + "\n")

if "previewGroup = new GroupBox();" not in s:
    init = ["            previewGroup = new GroupBox();", "            previewGrid = new DataGridView();"]
    init += [f"            previewColumn{i} = new DataGridViewTextBoxColumn();" for i in range(1, 19)]
...
if "previewGroup.Controls.Add(previewGrid);" not in s:
    ...
    block.append("                " + ", ".join(f"previewColumn{i}" for i in range(1, 19)) + ",")
    block += [
        "            });",
    ...
    for i in range(1, 19):
        block += [
            f"            previewColumn{i}.HeaderText = \"ستون {i}\";",
            f"            previewColumn{i}.Name = \"previewColumn{i}\";",
            f"            previewColumn{i}.ReadOnly = true;",
            f"            previewColumn{i}.Width = 120;"
        ]
    s = s.replace("            // \n            // deleteSymbolsButton\n", "\n".join(block) + "\n            // \n            // deleteSymbolsButton\n")
